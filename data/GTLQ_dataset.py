...
class GTLQDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        if opt['data_type'] == 'lmdb':
            ...
            # self.variation_paths = util.get_image_paths(opt['data_type'], opt['dataroot_variation'])  # varaition list
        else:
    ...
    def __getitem__(self, index):
        ...
        # variation_paths = self.variation_paths[index]
        ...
        ## load variations
        # variation = np.load(variation_paths)
        # Variation = torch.from_numpy(variation).float()
        
        # # don't need LR because it's generated from HR batches.
        # img_LR = torch.ones(1,1,1)

        # return {'LQ': img_LR, 'GT': img_GT, 'LQ_path': LR_path, 'GT_path': GT_path, 'Variation' : Variation}
